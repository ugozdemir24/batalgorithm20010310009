...
# =====================================================
def veri_al():
    """Collects the requested quantity for each box type from the user and expands those counts into the full box list used by the algorithm."""
    tum_koliler = []
...
def rotate(koli):
    """Returns a copied box with its dimensions randomly reordered so the search can test different valid 3D orientations."""
    yeni = deepcopy(koli)
...
def arac_uygun_mu(koli, arac_tipi):
    """Checks whether the box dimensions fit inside the selected vehicle type without exceeding depth, height, or width limits."""
...
def uygun_araclar(koli):
    """Builds the list of vehicle types that can physically contain the given box according to dimension constraints."""
    uygun = []
...
def en_kucuk_uygun_arac(koli):
    """Selects the smallest-capacity vehicle that can fit the box, helping reduce unused capacity when a fallback vehicle is needed."""
...
def yuksek_doluluk_arac_adaylari(koli):
    """Returns suitable larger-vehicle candidates for the box when the search is trying to favor higher overall load utilization."""
    uygunlar = uygun_araclar(koli)
...
def random_cozum(koliler, baslangic=False):
    """Creates a randomized solution for initialization or exploration by shuffling boxes, optionally rotating them, and assigning vehicle preferences."""
...
def hizli_araclara_dagit(cozum):
    """Performs a fast volume-based distribution across vehicles without calculating exact 3D coordinates, mainly for quick scoring."""
    arac_listesi = []
    ceza = 0
    sirali_cozum = sorted(
        cozum,
...
def hizli_fitness(cozum):
    """Scores a candidate solution quickly using vehicle count, unused volume, load-balance penalties, utilization rewards, and constraint penalties."""
...
def hizli_3d_decode(cozum):
    """Decodes a candidate solution into vehicles and approximate 3D coordinates using a faster placement attempt before the final detailed check."""
...
def gercek_3d_fitness(cozum):
    """Evaluates the solution with real 3D placement and adds collision, boundary, compactness, balance, and utilization effects to the fitness score."""
...
def final_3d_fitness(cozum):
    """Runs the final scoring pass on a candidate using actual 3D placement, vehicle count, unused volume, and load distribution quality."""
...
# =================================
def fitness(cozum):
    """Keeps backward compatibility for older calls by routing the generic fitness request to the fast fitness calculation."""
    return hizli_fitness(cozum)

# =====================================================
def fazli_fitness(cozum, faz_no=1, gercek_3d=False, faz_ilerleme=0.0):
    """Calculates either fast or real 3D fitness depending on the phase, then applies a confidence multiplier for staged optimization."""
    if gercek_3d:
...
def faz_bilgisi(iterasyon_indeksi, toplam_iterasyon, hedef_yarasa_sayisi):
    """Determines the current optimization phase, exploration/focus ratios, active bat counts, and normalized progress within that phase."""
...
def arac_plani_uret(toplam_hacim, maksimum_arac=6, hedef_doluluk=0.85):
    """Creates a baseline vehicle plan whose total capacity is close to the target utilization required for the total shipment volume."""
    ...
    def tara(indeks, kalan_sayi, secilen):
        """Recursively enumerates vehicle-type combinations and stores the plans that satisfy capacity and utilization requirements."""
...
def arac_plan_adaylari_uret(toplam_hacim, maksimum_arac=8, hedef_doluluk=0.75):
    """Generates alternative vehicle plans for the final placement stage, prioritizing plans that can achieve strong utilization."""
    tipler = list(araclar.keys())
    adaylar = []
    gevsek_adaylar = []
...
def planli_hacim_dagit(cozum):
    """Distributes boxes quickly according to a predefined vehicle plan using volume and vehicle-fit checks instead of exact coordinates."""
    toplam_hacim = sum(item["koli"]["h"] for item in cozum)
    plan = arac_plani_uret(toplam_hacim)
    arac_listesi = []
    ceza = 0
...
def arac_sec_bat(koli, freq, loudness, oran):
    """Chooses a vehicle preference for a box based on bat-algorithm parameters, balancing exploration with more focused exploitation."""
    uygunlar = uygun_araclar(koli)
...
def komsu(cozum, iterasyon, max_iter, pulse, loudness, velocity, freq, hedef_cozum, faz_no):
    """Creates a neighboring solution from the current one by applying changes that represent bat movement in the search space."""
    yeni = deepcopy(cozum)
...
# =====================================================
def iyilestir(cozum):
    """Attempts to improve a solution through local-search moves that tighten packing choices and correct weak vehicle assignments."""
    yeni = deepcopy(cozum)
...
def yarasa_algoritmasi(koliler, yarasa_sayisi=50, iterasyon=200):
    """Runs the phased bat algorithm, evaluates generated candidates, maintains good solutions, and searches for the best 3D packing result."""
    bats = []
    ...
    def arsive_ekle(kayitlar):
        """Stores strong candidates found during the first phase in a limited archive and keeps them ordered by fitness for later reuse."""
        nonlocal faz1_arsiv
...
def kutular_cakisiyor(a, b):
    """Checks whether two 3D boxes overlap by comparing their occupied coordinate intervals on all three axes."""
    return not (
        a["px"] + a["d"] <= b["px"] or
        b["px"] + b["d"] <= a["px"] or
        a["py"] + a["y"] <= b["py"] or
...
def arac_detay_olustur(tip):
    """Creates the detailed vehicle record for a vehicle type, including capacity, remaining volume, and the list of placed boxes."""
    return {
        "tip": tip,
        "kalan": araclar[tip][3],
        "kapasite": araclar[tip][3],
...
def koli_orientasyonlari(koli):
    """Generates the unique dimension permutations for a box so each valid 3D orientation can be considered once."""
    orientasyonlar = []
    gorulen = set()
...
def dikdortgen_kapsar(a, b):
    """Determines whether one free rectangle fully contains another, which helps remove redundant MaxRects spaces."""
    ax, az, ad, ag = a
    bx, bz, bd, bg = b
...
def serbest_dikdortgenleri_temizle(dikdortgenler):
    """Removes redundant or fully contained free rectangles from the MaxRects space list to keep placement candidates efficient."""
    temiz = []
...
def dikdortgen_bol(serbest, x, z, d, g):
    """Splits a free rectangle into the remaining free regions after a box footprint has been placed inside it."""
    sx, sz, sd, sg = serbest
    yeni = []
...
def dikdortgenler_kesisiyor(a, b):
    """Calculates whether two 2D rectangles intersect, used when updating available MaxRects floor areas."""
    ax, az, ad, ag = a
    bx, bz, bd, bg = b
    return not (
        ax + ad <= bx or
...
def tum_serbestleri_guncelle(serbestler, x, z, d, g):
    """Updates all free rectangles after a new placement by splitting intersecting spaces and pruning redundant regions."""
    yerlesen = (x, z, d, g)
    yeni = []
...
def arac_detay_katman_olustur(tip):
    """Prepares the vehicle detail structure with MaxRects layer data so boxes can be placed in stacked 3D layers."""
...
def maxrects_koli_yerlestir(arac, koli):
    """Places one box into the best available vehicle layer and free rectangle by testing orientations and choosing the lowest-cost fit."""
    tip = arac["tip"]
    D = araclar[tip][0]
    Y = araclar[tip][1]
    G = araclar[tip][2]
...
def cozum_sirasi_uret(cozum, mod):
    """Orders the boxes in a solution according to the selected placement mode before attempting the 3D packing pass."""
    if mod == "yarasa":
...
def maxrects_plan_dene(cozum, plan, mod, yeni_arac_ekle=False):
    """Attempts to place all boxes with the MaxRects method using a specific vehicle plan and ordering mode, optionally adding vehicles if needed."""
    araclar_detay = [arac_detay_katman_olustur(tip) for tip in plan]
    ceza = 0
    yerlesmeyen = 0
...
def maxrects_skorla(araclar_detay, ceza, yerlesmeyen):
    """Scores a MaxRects placement result using unplaced-box penalties, vehicle count, unused space, utilization quality, and balance penalties."""
    if len(araclar_detay) == 0:
        return 10**12
...
def maxrects_cozumden_arac_detay_uret(cozum):
    """Builds the best real 3D vehicle details for a solution by trying alternative vehicle plans and ordering modes."""
    toplam_hacim = sum(item["koli"]["h"] for item in cozum)
...
def yeni_arac_tipi_sec(koli, kalan_toplam, tercih):
    """Selects a suitable new vehicle type for a box that could not be placed, preferring the requested type or a sensible larger option."""
    uygunlar = uygun_araclar(koli)
...
def cozumden_arac_detay_uret(cozum):
    """Wrapper used by the final report to convert a solution into real 3D vehicle details through the MaxRects decoder."""
    return maxrects_cozumden_arac_detay_uret(cozum)

# =====================================================
def yerlesim_kontrol_et(araclar_detay):
    """Validates the final placement by checking every box for vehicle-boundary violations and pairwise collisions."""
    hata_sayisi = 0
...
def sonuc_tablosu_kaydet(araclar_detay):
    """Saves the vehicle and box coordinate report to a CSV file so the final placement can be reviewed outside the program."""
...
def renk_uret(etiket):
    """Generates a deterministic color for each box label so identical box types appear consistently in the 3D visualization."""
...
def kasa_ciz(ax, D, Y, G):
    """Draws the vehicle interior boundaries as a wireframe in the 3D plot to show the available loading volume."""
    kenarlar = [
        [(0, 0, 0), (D, 0, 0)],
        [(D, 0, 0), (D, Y, 0)],
        [(D, Y, 0), (0, Y, 0)],
...
def kutu_ciz(ax, x, y, z, dx, dy, dz, etiket):
    """Draws a single box as a labeled 3D prism using its placement coordinates, dimensions, and generated display color."""
    noktalar = [
        [x, y, z],
        [x+dx, y, z],
        [x+dx, y+dy, z],
...
def arac_3d_ciz(arac, arac_no):
    """Renders all boxes assigned to one vehicle as a 3D model and saves the visualization image for that vehicle."""
    tip = arac["tip"]
    D = araclar[tip][0]
    Y = araclar[tip][1]
    G = araclar[tip][2]
...
def sonuc_yaz(eniyi, sure):
    """Prints the best solution report, including vehicle usage, load ratios, coordinates, validation results, and generated 3D models."""
    print("\n")
    print("="*60)
    print("EN IYI COZUM")
    print("="*60)
...
def grafik_ciz(grafikler):
    """Plots the bat algorithm fitness convergence over iterations, saves the chart image, and displays it to the user."""
...
# =====================================================
def main():
    """Coordinates the full program flow: input collection, automatic parameter selection, algorithm execution, reporting, and convergence plotting."""
    print("\n")
    print("." * 50)
